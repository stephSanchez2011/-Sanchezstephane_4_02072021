(function () {
    ...
    const MONSTER_NAMES = [
        "Gobelin des cavernes", "Slime visqueux", "Araignée géante", "Squelette errant",
        "Orc sauvage", "Chauve-souris démoniaque", "Golem de pierre", "Spectre hurlant",
        "Hydre des marais", "Dragonnet"
    ];
    ...
    const els = {
        ...
        monsterBody: document.querySelector(".monster-body"),
    ...
    function getTapDamage() {
        ...
        return Math.floor((state.baseTapDamage + bonus) * relicMultiplier());
    }
    ...
    function getHeroDps(heroId) {
        ...
        return Math.floor(def.baseDps * hero.level * Math.pow(1.05, hero.level - 1) * relicMultiplier());
    }
    ...
    function spawnMonster() {
        ...
        var nameIdx = (state.stage - 1) % MONSTER_NAMES.length;
        els.monsterName.textContent = isBossStage(state.stage)
            ? "BOSS — " + MONSTER_NAMES[nameIdx]
            : MONSTER_NAMES[nameIdx];
    ...
    function dealDamage(amount, isCrit, x, y) {
        ...
        showDamageNumber(amount, isCrit, x, y);
        updateHud();
    ...
    function updateHud() {
        ...
        var pct = state.monsterMaxHp > 0
            ? (state.monsterHp / state.monsterMaxHp) * 100
            : 0;
    ...
    function renderShop() {
        ...
        HEROES.forEach(function (def) {
            ...
            li.innerHTML =
                ...
                '<div class="item-info">' +
                    ...
                    '<div class="item-desc">' + (dps > 0 ? formatNumber(dps) + " DPS" : "Recruter") + '</div>' +
                '</div>' +
                '<div class="item-action">' +
                    '<div class="item-cost">🪙 ' + formatNumber(cost) + '</div>' +
                    '<div class="item-level">Niv. ' + hero.level + '</div>' +
                '</div>';

            if (canBuy) {
                li.addEventListener("click", function () { buyHero(def.id); });
            }
        ...
        UPGRADES.forEach(function (upgrade) {
            ...
            li.innerHTML =
                ...
                '</div>' +
                '<div class="item-action">' +
                    '<div class="item-cost">' + (owned ? "✓ Acheté" : "🪙 " + formatNumber(upgrade.baseCost)) + '</div>' +
                '</div>';

            if (canBuy) {
                li.addEventListener("click", function () { buyUpgrade(upgrade.id); });
            }
    ...
    function gameLoop(now) {
        ...
        if (dt > 0 && dt < 1) {
            ...
            if (dps > 0 && state.monsterHp > 0) {
                dealDamage(dps * dt, false, 130, 130);
            }
        }
